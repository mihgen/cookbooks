...
set.hadoop.data_node.start_cmd = "#{hadoop_prestart}; echo Y | #{node[:hadoop][:core_dir]}/bin/hadoop namenode -format; ./hadoop-daemon.sh --config #{node[:hadoop][:conf_dir]} --script #{node[:hadoop][:core_dir]}/bin/hdfs start datanode"
set.hadoop.secondary_name_node.start_cmd = "#{hadoop_prestart}; ./hadoop-daemon.sh --config #{node[:hadoop][:conf_dir]} --script #{node[:hadoop][:core_dir]}/bin/hdfs start secondarynamenode"
